...
class IoTDevice(Thread):
    ...
    # Initialization
    def __init__(
        self, topic: str, devuuid: str, interval: float, modifier: float, print_logs: bool
    ):
        ...
        # UUIDs
        self.uuid = (
            re.sub(r"(\S{8})(\S{4})(\S{4})(\S{4})(.*)", r"\1-\2-\3-\4-\5", uuid.uuid4().hex)
            if devuuid == ""
            else devuuid
    ...
    def tic_behavior(self):
        ...
        while True:
            if not self.active:
                print(
                    f"{self.dev_class}[{self.uuid[0:6]}] inactive <N={self.msg_count} | T={tic - last_tic:.3f}s>",
                    kind="",
                )  # print info
            ...
            print(
                f"({self.topic}) <- {self.dev_class}[{self.uuid[0:6]}] msg published <N={self.msg_count} | T={tic - last_tic:.3f}s>",
                kind="info",
            )  # print info
            if self.print_logs:
                print(msg)  # print_device_data(msg['timestamp'],msg['data'])
            print()
...
class ConveyorBelt(IoTDevice):
    ...
    # Initialization
    def __init__(
        self, params, topic=prodline_root, devuuid="", interval=3, modifier=0.0, print_logs=False
    ):
    ...
    def gen_data(self):
        ...
        self.conveyor_belt["rotational_speed"] = get_new_sample(
            self.conveyor_belt["rotational_speed"]
        )
        ...
        conveyor_belt_copy["linear_speed"] = (
            self.conveyor_belt["linear_speed"] if self.conveyor_belt["status"] else 0.0
        )
        conveyor_belt_copy["rotational_speed"] = (
            self.conveyor_belt["rotational_speed"] if self.conveyor_belt["status"] else 0.0
        )
...
class TagScanner(IoTDevice):
    ...
    def gen_data(self):
        # RFID SCANNER MODULE
        self.rfid_scanner["product_id"] = (
            self.rfid_scanner["product_id"] if coin(0.7) else random.randint(0, 10)
        )
        self.rfid_scanner["process_id"] = (
            self.rfid_scanner["process_id"] if coin(0.7) else random.randint(0, 10)
        )
...
class ProductionControl(IoTDevice):
    ...
    def gen_data(self):
        ...
        self.production_control["production_status"] = (
            coin(0.95) if self.production_control["production_status"] else coin(0.2)
        )
...
class RepairControl(IoTDevice):
    ...
    def gen_data(self):
        ...
        self.repair_control["repair_status"] = (
            self.repair_control["repair_status"]
            if coin(0.8)
            else (self.repair_control["repair_status"] + 1) % 2
        )
...
class PoseDetector(IoTDevice):
    ...
    # Initialization
    def __init__(
        self, params, topic=prodline_root, devuuid="", interval=4, modifier=0.0, print_logs=False
    ):
        IoTDevice.__init__(self, topic, devuuid, interval, modifier, print_logs)
...
class AirQuality(IoTDevice):
    ...
    def gen_data(self) -> dict:
        return {
            ...
            "pressure_sensor": {
                "pressure": sample_normal_mod(self.gt.get("pressure"), modifier=self.modifier)
            },
            "air_quality_sensor": {
    ...
    def gen_data(self) -> dict:
        return {
            "temperature_humidity_sensor": {
                "temperature": sample_normal_mod(
                    self.gt.get("temperature"), modifier=self.modifier
                ),
                "humidity": sample_normal_mod(self.gt.get("humidity"), modifier=self.modifier),
            },
            "air_quality_sensor": {
...
class NoiseSensor(IoTDevice):
    ...
    # Initialization
    def __init__(
        self, topic=safetyenv_root, devuuid="", interval=4, modifier=0.0, print_logs=False
    ):
        IoTDevice.__init__(self, topic, devuuid, interval, modifier, print_logs)
...
class SeismicSensor(IoTDevice):
    ...
    # Simulate time series behavior around initial values
    def gen_data(self) -> dict:
        self.seismic_sensor["intensity"] = (
            random.randint(0, 1) if coin(0.95) else random.randint(2, 8)
        )
...
class RainSensor(IoTDevice):
    ...
    def gen_data(self) -> dict:
        return {
            "rain_sensor": {
                "cumdepth": sample_normal_mod(self.gt.get("rain_cumdepth"), modifier=self.modifier)
            }
        }
    ...
    def gen_data(self) -> dict:
        return {
            "wind_sensor": {
                ...
                "direction": sample_normal_mod(
                    self.gt.get("wind_direction"), modifier=self.modifier
                ),
            }
        }
