...
from datetime import datetime, timedelta  # noqa: F401  (re-exported for kgagent.py)
from json import JSONEncoder, dump, dumps, loads  # noqa: F401  (re-exported)
from threading import Thread
from typing import Any, Dict, List, Tuple  # noqa: F401  (re-exported typing aliases)
from uuid import uuid4  # noqa: F401  (re-exported for iotdevices.py)

import numpy as np
import pandas as pd
from benedict import benedict
from colorama import Fore, Style
from joblib import Parallel, delayed  # noqa: F401  (re-exported for kgagent.py)
from numpy import random

# Re-exported for the wildcard-import modules (kgagent.py, iotdevices.py)
from paho.mqtt import client as mqtt_client  # noqa: F401
...
class TypeDBClient:
    ...
    def initialization(self):
        ...
        if self.cli.databases().contains(kb_name):
            self.cli.databases().get(kb_name).delete()
        ...
        with open("typedbconfig/schema.tql") as f:
            self.define_query(f.read())
        print(f"{kb_name} SCHEMA DEFINED.", kind="success")

        # Open a DATA session to populate kb with initial data
        with open("typedbconfig/data.tql") as f:
    ...
    def get_integrated_devices(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            k: {"class": "", "integrated": True, "period": 0, "timestamps": [], "modules": {}}
            for k in dev_uuids
        }
...
class SDFManager:
    ...
    def build_sdf(self, dev_class: str) -> tuple[dict[str, Any], pd.DataFrame]:
        ...
        with open(self.path + "/" + dev_class + ".sdf.json") as sdf_file:
            inner_sdf = benedict(loads(sdf_file.read()))
        ...
        for path, sdfRef in paths.items():
            ...
            if filename == "#":  # reference to an inner sdf file path
                ...
                if filename not in self.sdf_cache:  # add sdf to cache if not there
                    with open(self.path + filename + ".sdf.json") as sdf_file:
        ...
        for sdfThing, thing_dic in sdf["sdfThing"].items():
            ...
            for sdfObject, object_dic in thing_dic["sdfObject"].items():
                ...
                for sdfProperty, prop_dic in object_dic["sdfProperty"].items():
                    ...
                    prop_unit = prop_dic.get("unit", None)
                    rows.append(
                        (
                            sdfThing,
                            thing_desc,
...
def calc_voting_result_df(votes: list[dict[str, int]]) -> pd.DataFrame:
    ...
    return pd.DataFrame(total_vote_sdf.items(), columns=["candidate", "score"]).sort_values(
        by="score", ascending=False
    )


# Compute closest classes by comparing SDF descriptions
def get_closest_classes(
    noninteg_class: pd.DataFrame,
    integ_classes: pd.DataFrame,
    i: int,
    score: int = 3,
) -> dict[str, int]:
    ...
    integ_classes["str_dist"] = integ_classes.apply(
        lambda x: calc_str_dist(non_integ_class_row_desc, x), axis=1
    )
    closest_things = integ_classes[["thing", "obj", "prop", "str_dist"]].sort_values(
        by="str_dist", ascending=False
    )
...
def get_closest_devs(
    ...
    for i, integ_dev_row in integ_devs.iterrows():
        ...
        if inspected_series.size < query_series.size:
            continue
...
def sample_triangular(offset: float, amp: float, T: float, phi: float) -> float:
    ...
    val = (
        offset + 2 * amp * ((t % (T / 2) / T) - 0.5)
        if t % T < T / 2
        else offset - 2 * amp * ((t % (T / 2) / T) - 0.5)
    )
...
# Generate robot data
def gen_robot_data(
    offset: float, A: float, T: float, phi: float, actuator_status: bool
...
types_trans = {"number": "double", "string": "string", "boolean": "boolean"}
