...
class KGAgent(TypeDBClient):
    ...
    def __init__(self, initialize=True, print_queries=False, buffer_th=60):
        ...
        self.buffer_th = (
            buffer_th  # values within buffer_th last minutes will be stored for each device
        )
        self.sdf_dicts = {}
    ...
    def on_message(self, client, userdata, msg):
        ...
        match msg["category"]:
            ...
            case "DISCONNECTED":
                print(
                    f"({topic}) - {dev_class}[{uuid[0:6]}] disconnected from broker.", kind="fail"
                )

            case "DATA":
                if uuid not in self.dev_msg_stats:
                    self.dev_msg_stats[uuid] = [0, 0]
                print(
                    f"({topic}) -> {dev_class}[{uuid[0:6]}] msg received <N={self.dev_msg_stats[uuid][0] + 1}>",
                    kind="info",
                )
                ...
                print(
                    arrow_str
                    + f"msg processed <Tp={(toc - tic) * 1000:.0f}ms | Avg.Tp={(self.dev_msg_stats[uuid][1] / self.dev_msg_stats[uuid][0]) * 1000:.0f}ms>\n",
                    kind="info",
                )
                ...
                if self.total_msg_count % 100 == 0:
                    ...
                    print("-----------------------------------------------------", kind="summary")
                    print(
                        f"MSGs SUMMARY <N={self.total_msg_count} | Avg. Tp={(self.msg_proc_time / self.total_msg_count) * 1000:.0f}ms>",
                        kind="summary",
                    )
                    print("-----------------------------------------------------\n", kind="summary")
    ...
    def define_modules_attribs(self, dev_class: str, uuid: str, timestamp: str, data: dict) -> None:
        ...
        print(
            arrow_str + f"modules/attribs defined <Tq={(toc - tic) * 1000:.0f}ms>", kind="success"
        )
    ...
    def consistency_handler(self, msg: dict) -> None:
        ...
        if not self.devices[uuid]["integrated"] and len(self.devices[uuid]["timestamps"]) > 20:
        ...
        self.devices[uuid]["period"] = (
            dt_timestamp - self.devices[uuid]["timestamps"][-1]
        ).total_seconds()
        self.devices[uuid]["timestamps"].append(dt_timestamp)
    ...
    def integrate(self, dev_class: str, uuid: str, dt_timestamp: datetime) -> None:
        ...
        integ_devs = devs_df[(devs_df.integ) & (devs_df.uuid != uuid)]

        # Compute Top 5 closest SDF classes
        tic = time.perf_counter()
        votes = Parallel(n_jobs=12)(
            delayed(get_closest_classes)(noninteg_class, integ_classes, i)
            for i in range(noninteg_class.shape[0])
        )
        ...
        tic = time.perf_counter()
        votes = Parallel(n_jobs=12)(
            delayed(get_closest_devs)(noninteg_dev, integ_devs, closest_classes, i)
            for i in range(noninteg_dev.shape[0])
        )
        voting_result_df = calc_voting_result_df(votes)
        ...
        print(
            arrow_str
            + f"device integrated (relations replicated in KG) <Tq={(toc - tic) * 1000:.0f}ms>",
            kind="success",
        )
        ...
        if self.devices[integ_uuid]["timestamps"][-1] < (
            dt_timestamp - timedelta(seconds=2 * self.devices[integ_uuid]["period"])
        ):
            tic = time.perf_counter()
            ...
            print(
                arrow_str
                + f"old device and its modules disintegrated from KG <Tq={(toc - tic) * 1000:.0f}ms>",
                kind="success",
            )
