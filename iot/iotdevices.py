...
class ConveyorBelt(IoTDevice):
    ...
    def gen_data(self) :
        # THE PARAMETERS USED IN THE DATA GENERATION SHOULD BE PASSED ON THE CONSTRUCTOR, TO GIVE
        # PERSONALITY TO THE DATA GENERATED BY THE DEVICE BASED ON ITS LOCATION AND FUNCTION
        status = random.uniform() < 0.9
        return {
            'conveyor_belt' : {
                'status' : status,
...
class ProductionControl(IoTDevice):
    ...
    # Simulate data generation
    def gen_data(self) :
        return {'production_control' : {'production_status' : random.uniform() < 0.7}}
...
class ConfigurationScanner(IoTDevice):
    ...
    def gen_data(self) :
        return {
            'left_cam': {'config_status' : random.uniform() < 0.975},
            'right_cam': {'config_status' : random.uniform() < 0.975},
            'front_cam': {'config_status' : random.uniform() < 0.975},
            'back_cam': {'config_status' : random.uniform() < 0.975},
            'top_cam': {'config_status' : random.uniform() < 0.975},
            'bottom_cam': {'config_status' : random.uniform() < 0.975}
        }
...
class QualityScanner(IoTDevice):
    ...
    def gen_data(self) :
        return {
            'left_cam': {'quality_status' : random.uniform() < 0.975},
            'right_cam': {'quality_status' : random.uniform() < 0.975},
            'front_cam': {'quality_status' : random.uniform() < 0.975},
            'back_cam': {'quality_status' : random.uniform() < 0.975},
            'top_cam': {'quality_status' : random.uniform() < 0.975},
            'bottom_cam': {'quality_status' : random.uniform() < 0.975}
        }
...
class PickUpRobot(IoTDevice):
    ...
    def gen_data(self) :
        ...
        if self.n_actuator%2 == 0 & self.n_actuator != 0 :
            self.n_actuator = 0
            self.actuator_status = random.uniform() < 0.4
