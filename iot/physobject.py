...
The data generated by each of these devices is personalized depending on the tasks they belong to using the data generation
parameters. This way, we will give the same data generation parameters to devices performing similar tasks, so that their behavior is
somehow similar. 

Additionally, ground truth classes are instantiated, which simulate variables such as the temperature or the pressure for different spaces,
such as indoors and outdoors in this case. This way, if two temperature sensors are placed indoors, they will be linked to the same
ground truth instance and therefore the data they generate will be somehow similar (with some random noise).

Finally, some cases of interest are simulated, such as the replacement of a device for a new one belonging to a slightly
different class, and the installation of a complementary device in order to speed up a task. This cases of interested have been
designed to test the correct implementation of the KG Agent functionalities.
"""
# ---------------------------------------------------------------------------
# Imports
